    ...
    <div class="workingSpace">
      <div class="loggedinDiv">
        <div v-if="state.loggedin" class="loggedInTitle">
          ...
          <a :href="'/b/?i=/u/' + state.loggedinUserName" v-html="truncate(state.loggedinUserName)"></a>
        </div>
        <Account :state="state"/>
      </div>
      <div class="infoButton" title="about whitehot robot" @click="launchInfoPage()"></div>
      <a :href="origin + '/b/'" class="appName">
...
export default {
  ...
  methods:{
    launchInfoPage(){
      ...
      a.setAttribute('href', 'https://code1.whitehot.ninja/a/about')
    ...
    createPost(){
      ...
      fetch(this.state.baseURL + '/createPost.php',{
        ...
        console.log(data)
        if(data[0]) window.location.href = this.state.baseURL + '/b/?i=/post/' + this.state.decToAlpha(data[1])
      })
    }
  },
  mounted(){
